...
class DispatchQueueIN extends Bundle {
  ...
  val rs_num_in       = Vec(2, Input(UInt(log2Up(RSNum+1).W)))
  val rs_can_allocate = Vec(RSNum+1, Input(Bool()))
}
...
class DispatchQueue extends Module with Config with HasCircularQueuePtrHelper {
  ...
  val rs_num    = RegInit(VecInit(Seq.fill(DispatchQueueSize)(0.U(log2Up(RSNum+1).W))))
